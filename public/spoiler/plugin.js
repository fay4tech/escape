...
tinymce.PluginManager.add('spoiler', function(editor, url)
{
  ...
  function addSpoiler()
  {
    ...
    if (node) {
      ...
      if (!content) {
        content = 'text a Spoiler.';
      }
      selection.setContent('<div class="spoiler">' +
                           '<div class="spoiler-toggle"><button type="button" class="btn btn-sm btn-outline-danger mb-1" >' + spoilerCaption +'</button></div>' +
                           '<div class="spoiler-text alert alert-danger mt-2" role="alert">' + content + '</div></div>');
      });
      editor.nodeChanged();
    }
  }
